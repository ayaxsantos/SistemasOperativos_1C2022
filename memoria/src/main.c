...
void setear_estructuras_de_memoria() {
    ...
    if (config_memoria.tamanio % config_memoria.tamanio_pagina == 0){
		memoria_principal->memoria = malloc(config_memoria.tamanio);
		memoria_principal->tamanio_frame = config_memoria.tamanio_pagina;
		memoria_principal->cantidad_frames = config_memoria.tamanio / config_memoria.tamanio_pagina;
    
        iniciar_particionamiento_en_frames();
        setear_algoritmo_reemplazo();
    } else {
    	log_error(logger_memoria,"No se pudieron setear las estructuras de memoria. Error en el archivo config.");
    };
}
